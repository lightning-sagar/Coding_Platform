...
const QuestionView: React.FC<QuestionViewProps> = ({ darkMode }) => {
  ...
  const updateTestCasesWithResults = (results: TestResult[]) => {
    ...
    setVisibleTestCases(prev =>
      prev.map((testCase, index) => {
        ...
        if (result) {
          console.log(result.correct, result.result, result.timeout, result.error, result);
          return {
            ...testCase,
            status: result.correct ? "pass" : "fail",
            actualOutput: result?.error ? result.error
              .replace(/^.*?\.cpp:\d+:\d+:\s*/gm, '')
              .trim()
              : result.result.trim(),
            executionTime: result.timeout ? result.timeout : "0",
          };
        }
        return testCase;
      })
  ...
  return (
    ...
    >
      {/* Header */}
      <header
        className={`border-b sticky top-0 z-50 backdrop-blur-sm transition-colors duration-300 ${darkMode ? "bg-gray-800/95 border-gray-700" : "bg-white/95 border-gray-200"
          }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <button
              onClick={() => navigate(-1)}
              className={`flex items-center px-3 py-2 rounded-lg transition-all hover:scale-105 ${darkMode
            ...
            <div className="flex items-center space-x-6">
              <div
                className={`flex items-center px-3 py-2 rounded-lg text-sm font-medium ${darkMode ? "text-gray-300 bg-gray-700" : "text-gray-600 bg-gray-100"
                  }`}
              >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-12 gap-6 ">
          ...
          <div className="col-span-12 lg:col-span-5 space-y-6">
            <div
              className={`rounded-xl shadow-lg border p-6 h-full overflow-y-auto transition-all duration-300 hover:shadow-xl ${darkMode
            ...
            >
              ...
              <div className="prose prose-sm max-w-none">
                <div
                  className={`whitespace-pre-wrap leading-relaxed text-base ${darkMode ? "text-gray-300" : "text-gray-700"
                    }`}
                >
                  {question.desc}
                </div>
              ...
              >
                <h3
                  className={`font-bold text-lg mb-4 ${darkMode ? "text-white" : "text-gray-900"
                    }`}
                >
                  Sample Input/Output
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label
                      className={`block text-sm font-semibold mb-3 ${darkMode ? "text-gray-300" : "text-gray-700"
                        }`}
                    >
                      Input
                    </label>
                    <pre
                      className={`border-2 rounded-xl p-4 text-sm font-mono transition-all hover:shadow-md whitespace-pre-wrap break-words overflow-auto max-h-60 ${darkMode
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
            <div
              className={`rounded-xl shadow-lg border p-6 h-full flex flex-col transition-all duration-300 hover:shadow-xl ${darkMode
                ? "bg-gray-800 border-gray-700"
                : "bg-white border-gray-200"
                }`}
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <select
                  ...
                  className={`px-4 py-2 border-2 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all ${darkMode
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div
                  className={`border-2 border-dashed rounded-xl p-8 text-center transition-all flex-1 flex flex-col justify-center hover:scale-105 ${file
                ...
                >
                  ...
                  >
                    <div className={`p-4 rounded-full mb-4 ${file
                    ...
                    <span
                      className={`text-lg font-semibold mb-2 ${file
              ...
              {/* Submission Status */}
              {apiResponse && (
                <div className={`mt-4 p-4 rounded-lg border-2 ${apiResponse.allPassed
                  ...
                  <div className="flex items-center">
                    ...
                    <span className={`font-semibold ${apiResponse.allPassed
                      ? "text-green-600"
                      : "text-red-600"
                      }`}>
                      {apiResponse.message}
              ...
              >
                <button
                  ...
                  className={`w-full flex items-center justify-center px-6 py-4 rounded-xl font-semibold transition-all transform hover:scale-105 ${!file || submissionStatus === "running"
            ...
            >
              ...
              <div className="space-y-4 mb-6">
                {visibleTestCases.map((testCase, index) => (
                  <div
                    key={testCase.id}
                    className={`border-2 rounded-xl p-4 transition-all hover:shadow-md ${getStatusBorder(testCase.status)
                      }`}
                  >
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <pre
                          className={`text-xs p-3 rounded-lg border font-mono mt-1 transition-colors ${darkMode
                            ? "bg-gray-900 border-gray-600 text-gray-300"
                            : "bg-gray-50 border-gray-200 text-gray-900"
                            }`}
                        >
                      ...
                      {testCase.actualOutput && (
                        <div>
                          <label
                            className={`text-xs font-semibold uppercase tracking-wide ${testCase.status === "pass"
                              ? "text-green-500"
                          ...
                          <pre
                            className={`text-xs  font-mono mt-1 transition-colors w-fit max-w-full inline-block overflow-auto whitespace-pre-wrap break-words ${testCase.status === "pass"
                              ? (darkMode
                                ? "  text-green-300"
                                : " text-green-800")
                              : testCase.status === "fail"
              ...
              {hiddenTestCases.length > 0 && (
                <div>
                  <h3
                    className={`text-sm font-semibold mb-4 ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                  >
                  ...
                  <div className="max-h-60 overflow-auto pr-1">
                    <div className="grid grid-cols-2 gap-3">
                      {hiddenTestCases.map((testCase, index) => (
                        <div
                          key={testCase.id}
                          className={`flex items-center justify-between p-3 rounded-lg border-2 transition-all ${getStatusBorder(testCase.status)}`}
                        >
                          <span
                          ...
                          <div className="flex items-center space-x-2">
                            {testCase.executionTime && (
                              <span className={`text-xs px-1 py-0.5 rounded ${darkMode ? "bg-gray-700 text-gray-400" : "bg-gray-100 text-gray-500"}`}>
                                {testCase.executionTime}s
                              </span>
                            )}
                            {getStatusIcon(testCase.status)}
